---
title: "The Terminal That (Almost) Never Dies: Building a Persistent Terminal Service for Electron"
...
## Architecture: A detachable Service
...
With this architecture, the Electron app becomes just a *client* of the terminal service. It can:

- Restart the app → Reconnect to running sessions
- Open multiple windows → All attach to the same service
- Crash and recover → Cold restore from disk history

### Spawning the Service

The service is a Node.js process spawned with a clever Electron trick:
...
service.unref();  // Don't wait for service to exit
```
...
Communication between the main process and service uses newline-delimited JSON over Unix domain sockets:
...
async createOrAttach(params: CreateSessionParams) {
  // 1. Already in service? Just attach.
  if (service.hasSession(params.sessionId)) {
    return service.attach(params.sessionId);
  }

  // 2. Not in service, but history on disk? Cold restore.
...
### Cold Restore: Recovering from Service crashes

When the service dies (machine reboot, crash, kill -9), sessions are lost. But we still have disk history:
...
Each service session runs a headless xterm.js emulator. This might seem redundant—why emulate if there's no screen?
...
When we introduced the two-socket split, existing services couldn't speak the new protocol. We handle this gracefully:

```typescript
// Client detects version mismatch
if (response.protocolVersion !== EXPECTED_VERSION) {
  await shutdownStaleService();
...
When the Electron app closes, we *don't* kill service sessions:
...
async cleanup() {
  ...
  // Disconnect from service, but DON'T send kill
  this.disposeClient();
...
This is the whole point of the service architecture. The default should be persistence, not cleanup.

### 4. Concurrency Limits Prevent Spawn Storms

Opening a workspace with 10 terminal panes would previously spawn 10 sessions simultaneously, overwhelming the service. We added a semaphore with priority:
...
Today, `LocalTerminalRuntime` wraps our service. Tomorrow, `CloudTerminalRuntime` could wrap SSH connections or remote tmux sessions—same interface, different backend. The renderer doesn't need to know where the terminal lives.
