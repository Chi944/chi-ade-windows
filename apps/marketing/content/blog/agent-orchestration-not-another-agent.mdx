...
Agent sessions should survive crashes, app restarts, and laptop sleep cycles. A background service that owns the sessions independently of the UI solves this — the same pattern that tmux uses for terminal multiplexing, applied to agent orchestration.
...
- **Persistent service** for session management (Unix domain sockets, survives crashes)
