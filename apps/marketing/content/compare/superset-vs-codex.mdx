...
Superset is a desktop terminal that orchestrates CLI-based coding agents in parallel across isolated Git worktrees. Each agent — Codex, Claude Code, Aider, OpenCode, or any CLI process — gets its own branch and working directory. A persistent service manages sessions so they survive app restarts and crashes. Includes a built-in diff viewer and integrates with VS Code, Cursor, JetBrains, and Xcode. Open source (Apache 2.0) with zero telemetry.
...
Codex CLI sessions end when the terminal closes. Superset's persistent service keeps sessions alive across crashes and restarts. Start a long-running Codex task, close your laptop, and it's still running when you come back.
