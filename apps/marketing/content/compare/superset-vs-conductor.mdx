...
| **Terminal architecture** | Persistent service (survives app restarts) | Standard sessions (do not survive restarts) |
...
Superset is a desktop terminal for orchestrating AI coding agents. It runs any CLI-based agent -- Claude Code, Codex, OpenCode, Aider, or custom scripts -- in its own Git worktree. A persistent service manages all sessions via Unix domain sockets, so they survive app restarts and crashes. Each worktree gets a built-in diff viewer and one-click opening in VS Code, Cursor, JetBrains, or Xcode. Open source (Apache 2.0) with zero telemetry.
...
Superset runs a background service that owns all PTY sessions independently of the Electron app. Close Superset, reopen it, and your agents are still running. The service handles backpressure and priority-based concurrency (focused pane gets priority, background tabs hydrate gradually). Conductor's sessions persist when switching workspaces but do not survive app restarts -- a real limitation for long-running agent tasks.
