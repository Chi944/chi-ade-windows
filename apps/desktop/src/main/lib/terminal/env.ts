...
const ALLOWED_ENV_VARS = new Set([
	...
	"CODEX_HOME",
	"CLAUDE_CONFIG_DIR",
	"ADE_CODEX_INSTRUCTIONS_CONFIG_PATH",
	// Workspace-scoped capability for the loopback coordination server. This is
	// generated by ADE, not inherited from Electron's process environment.
	"ADE_COORDINATION_TOKEN",
...
export function setCoordinationTokenResolver(
	resolver: ((workspaceId: string) => string) | null,
): void {
	coordinationTokenResolver = resolver;
}
...
export function buildTerminalEnv(params: {
	...
	if (coordinationTokenResolver) {
		terminalEnv.ADE_COORDINATION_TOKEN = coordinationTokenResolver(workspaceId);
	}
	...
	for (const key of Object.keys(terminalEnv)) {
		...
		if (
			...
			normalizedKey === "ADE_CODEX_INSTRUCTIONS_CONFIG_PATH"
		) {
			delete terminalEnv[key];
		}
	}

	// Codex normally isolates config + AGENTS.md under a per-agent CODEX_HOME.
	// The separate instructions path keeps workspace memory available even when
	// account selection later replaces or removes CODEX_HOME for authentication.
	const usesCodexHome =
	...
	if (usesCodexHome) {
		...
		if (runtime === "codex") {
			terminalEnv.ADE_CODEX_INSTRUCTIONS_CONFIG_PATH =
				getAgentCodexInstructionsConfigPath(workspaceId);
		}
